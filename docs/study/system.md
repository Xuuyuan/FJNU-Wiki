...
!!! warning "提醒"
    ...
    福建省内几乎所有高校（包括福师大）的公费师范专业于 2026 年起均暂停招生，故今后不再有公费师范专业。
...
- 缓考成绩视同正考，正常计算绩点，在成绩单上与正考同一栏显示（如下图"成绩"列）。
- 补考成绩正常计算绩点，在成绩单上单列一栏显示（如下图"补考"列）。
- 部分学院在进行专业分流、评优评先、基地班选拔等工作时可能采取包括但不限于以下方案：
- - 计算成绩仅计算正考成绩/最高分成绩。
- - 补考合格者以 60 分计入。
- - 限定未有补考&重修经历者才可参与。

![成绩单示例](./pics/system-scoretable-example.png)  
*成绩单示例，图中第二门课程绩点以1.0计入；第三门课程未通过，绩点以0.0计入；第六门课程重修通过，绩点以1.0（对应60分）计入*
...
- 课程重修人数达到独立开班条件的，开设课程重修班，上课时间安排在周末或假期；无法达到独立开班条件的，随下一年级相同或相近专业插班重修（对于《大学物理》等公共课，重修报名时可以自行选择插入班级）。《高等数学》、《大学英语》重修单独开班。
...
- 重修次数不限。
- 重修成绩正常计算绩点，在成绩单中单列一栏显示（如下图"重修"列）。
- 部分学院在进行专业分流、评优评先、基地班选拔等工作时可能采取包括但不限于以下方案：
- - 计算成绩仅计算正考成绩/最高分成绩。
- - 重修合格者以 60 分计入。
- - 限定未有重修经历者才可参与。
...
![成绩单示例](./pics/system-scoretable-example.png)  
*成绩单示例，图中第六门课程重修通过，绩点以1.0（对应60分）计入*
...
培养方案是学校或职业院校为实现专业人才培养目标而制定的系统性指导文件，规定了课程设置、学分要求、培养目标及实施方式，以一份 PDF 文件/ Excel 表格的形式展示。每位本科生都可以通过[教务系统](https://jwglxt.fjnu.edu.cn)查询自己当前专业的培养方案（入口：信息查询 - 学生学业情况查询）及校内任意年级、任意专业的培养方案（入口：信息查询 - 教学执行计划查看 - 设置年级、学院 - 点击“查询” - 勾选想要查看的**一个**专业 - 点击顶部的“修读要求”、“课程信息”或“执行计划预览”）。

部分情况下，通过“学生学业情况查询”无法查询到自己的培养进程（提示暂无培养方案），此时只能等待教学秘书统一处理或使用“教学执行计划查看”功能替代。
