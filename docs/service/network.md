...
**校园流量卡并非强制要求办理，请根据自己的需求进行选择。** 校园流量卡与查成绩、选修课、访问教务系统、选课抢课均无关联。若有人以此类话术建议你办卡，请万分警惕！他人邀请你办卡可以得到 100-150 元/张 不等的提成，而“羊毛出在羊身上”的道理大家都懂。若确认有办卡需要，可以通过入学期间的各种上门推销或自行前往校内的运营商营业厅办理。
...
校园网是通过线下营业厅购买对应的套餐（或办理[校园流量套餐卡](#campus-sim-card)所另叠加的福州高校宽带电视融合套餐），通过教学楼、宿舍楼全覆盖的 WiFi 连接即可，离开学校时无法使用。电信/联通/移动并无本质区别。学校不允许学生自行接入宽带，通过运营商的官方APP申请办理宽带时会被告知无法办理。

**校园网并非强制要求办理，请根据自己的需求进行选择。**

!!! warning "校园网相关限制提醒"
    1. 2025年6月13日起，校园网屏蔽了 RDP 协议、 ~~SSH 服务 以及大量代理协议（仅 Hy2、Trojan正常）~~，此举导致通过校园网连接时几乎无法使用任何远程控制服务。经实测，2026年5月前对 SSH 服务的屏蔽已解除。
    ...
    5. 2026年8月起，网络中心对代理上网行为的监测进一步增强。目前已知此前存活的代理协议均被校园网策略所阻断，实测目前 Vless+UDP/XUDP 方案可用。

### 网络速率 {#cn-speed}

校园网套餐提供 30（包月）/100（包年）Mbps 的档位，和校园流量卡绑定的套餐提供 100Mbps 的档位。实际速率视实际环境而有所不同，一般教学周时 100Mbps 也只能跑到 80Mbps 左右，30Mbps 能跑到 40Mbps。**提醒：100Mbps = 12.5MB/s**

### 设备限制 {#cn-device-limit}

每个账号最多 2 台设备同时在线（每台路由器、笔记本电脑、手机、平板等均计为 1 台）。

### 购买方法 {#cn-buy}

=== "线下购买"
    校园网有运营商单宽套餐和融合套餐，供同学们自主按需选择。单宽套餐及融合套餐均需要携带身份证等证件至校园网服务中心、各运营商校内营业厅实名办理。单宽套餐为 35 元/月, 30Mbps 速率, 限 2 设备；360 元/年, 100Mbps 速率, 限 2 设备。融合套餐均为 59 元/月, 100Mbps 速率, 限 2 设备。

    | 服务中心名称 | 指引 |
    | --- | --- |
    | 校园网服务中心 | 桂苑5号楼（数统学院宿舍）旁一站式服务大厅 - 咨询热线：4008099099、13067220181 |
    | 移动营业厅 | 旗山校区文化街入口处"文化街"招牌底下、仓山校区原聚星园餐厅二楼店面 |
    | 电信营业厅 | 旗山校区文化街库迪咖啡对面、仓山校区原聚星园餐厅二楼A07（天兴超市旁） |
    | 联通营业厅 | 旗山校区文化街芒果优选超市对面、仓山校区原聚星园餐厅二楼（天兴超市出口处） |

    ??? tip "参考数据之网络与信息中心通知（2025年9月9日）"
...
=== "线上购买"
    ...
    根据[《关于变更校园网办理方式的通知》](https://mp.weixin.qq.com/s/_5az_grUMqaamfOHdHMf4A)，自2026年8月1日起，校园网不再支持线上购买。
...
=== "无线上网"
    ...
    | 验证方式 | 连接方法 | 区别 |
    | --- | --- | --- |
    | webportal | 连接名为 `webportal.fjnu.edu.cn` 的 WiFi 后会自动弹出登录页面（或者手动访问 [2.2.2.2](http://2.2.2.2) ），输入学号、校趣多的密码进行登录即可。 | 不支持无感身份认证（例：从宿舍到教室后需要重新登录），若连接失败会显示失败的原因 |
    | 1x | 连接名为 `1x.fjnu.edu.cn` 的 WiFi，身份为学号、匿名身份留空、密码为校趣多的密码、阶段二身份和 CA 证书选择无或不验证，其它信息保持默认。 | 支持无感身份认证（例：从宿舍到教室后依然可以直接连接），若连接失败不显示原因 |
...
=== "通过路由器连接校园网"
    ...
    路由器连接校园网原理 同 有线上网，只是 mentohust 代替了锐捷认证客户端完成认证。具体方法请自行了解，搜索关键词：mentohust。该方法原则上不受校园网规则允许，可能触发封禁（见：[校园网相关限制提醒](#cn-info)）。

    受线路所限，学校所有宿舍楼（旗山校区李苑 9-11 号楼除外）的网络连接速率最高到达 100Mbps，单线多播无法突破此上限。

### 校园网设备管理 {#cn-manage}

1. 访问卓智校园网自助服务系统 [公网地址](http://125.77.120.185:8181/z-os/self/)  / [内网地址（仅校园网可访问）](http://10.136.10.12:8080/z-os/self/)。
2. 在用户界面即可进行配置运营商、修改密码、踢出设备、查询购买记录、账务流水、查看及修改 MAC 绑定信息等操作。

### 在未购买校园网的情况下访问校内网站 {#login-without-campus-network}

部分校内网站（如[实验室安全教育考试](https://syaqjy.fjnu.edu.cn/fjnu_ksxt)、[体测系统](https://tcxt.fjnu.edu.cn)、[校园地图](https://gfgl.fjnu.edu.cn/iams/home.html#/index)、食堂人流量、晚查寝打卡系统）限制了只有使用校园网才能访问，但同样有替代方法可用。以下是一些参考方案：
